...
| Listado de ventas con filtros | `processes.md` (Reportes) | (no documentado) | `listar_ordenes_venta` con filtros de búsqueda y estado en `sales.html` | `implementado v1` |
...
| Estados de venta `Pending` y `Cancelled` con flujo real | `processes.md` (ciclo de vida) | `erd.md` (`status` de `SALES_ORDER`); `init_db.py` (default `'pending'`) | En v1, el estado de la venta es siempre `'Paid'`; los filtros innecesarios de la UI han sido retirados | `implementado v1` |
...
| Metricas de ventas en dashboard | `processes.md` (Reportes); `requirements.md` RF-09 | (no modelado) | `obtener_metricas_dashboard()` e integrado en `dashboard.html` | `implementado v1` |
