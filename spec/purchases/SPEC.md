...
| Crear proveedores desde la UI | `processes.md` linea 49; `procedures.md` (Registro de compra, paso 2) | (no modelado) | Ruta POST `/purchases/suppliers` y modal en la UI implementados | `implementado v1` |
...
| Transicion `Pending -> Cancelled` (nueva en v1) | (no documentado) | (no modelado) | Ruta `/purchases/<id>/cancel` y función `cancel_purchase_order` implementadas | `implementado v1` |
...
| Filtro "Canceladas" en el sidebar | (no documentado) | (no modelado) | Filtro "Canceladas" agregado al sidebar y soportado en el listado | `implementado v1` |
| Constantes de estado centralizadas | (no documentado) | (no modelado) | Constantes importadas desde `constants.py` | `implementado v1` |
